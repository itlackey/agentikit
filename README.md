...
- **Manage sources** — add local dirs, git repos, npm packages, and websites as searchable asset sources [(details)](docs/guides/bundles.md)
  ```sh
  akm bundle add github:owner/repo         # GitHub
  akm bundle add https://docs.example.com  # crawled website
  ```
- **Search a unified index** — one FTS5 index across all your sources [(details)](docs/guides/discover-and-load.md)
  ```sh
  akm search "deploy" --type script --limit 5
  ```
- **Curate a shortlist** — get the best-match assets for a task without knowing exact names [(details)](docs/guides/discover-and-load.md)
  ```sh
  akm curate "set up a kubernetes deployment"
  ```
- **Load assets on demand** — show the full content of any asset by ref [(details)](docs/guides/discover-and-load.md)
  ```sh
  akm show workflows/ship-release
  ```
- **Capture local knowledge** — save discoveries as memories or imported docs [(details)](docs/guides/capture-knowledge.md)
  ```sh
...
- **Run structured workflows** — execute and verify resumable multi-step procedures [(details)](docs/reference/workflows.md)
  ...
  ```
- **Improve continuously** — feedback drives proposals; proposals drive asset quality [(details)](docs/guides/improve-the-library.md)
  ```sh
...
| Feature | Description |
| --- | --- |
| [Search & Discovery](docs/guides/discover-and-load.md) | Build the index, search, curate a shortlist, and load assets by ref |
| [Knowledge Management](docs/guides/capture-knowledge.md) | Capture memories, import docs, manage wikis, and store protected env/secret assets |
| [Sources & Registries](docs/guides/bundles.md) | Connect local dirs, git repos, npm packages, and websites; browse the registry |
| [Workflows](docs/reference/workflows.md) | Structured multi-step procedures with resumable run state |
| [The Improvement Loop](docs/guides/improve-the-library.md) | Feedback, history, proposals, and automated asset improvement |
| [Agent Integration](docs/guides/use-with-any-agent.md) | Wire akm into Claude Code, OpenCode, Cursor, and other coding assistants |
...
| [Bundle Authoring](docs/guides/author-bundles.md) | Build, publish, and share your own bundles |
