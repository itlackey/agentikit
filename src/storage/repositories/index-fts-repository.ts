...
function runFtsQuery(
  ...
  // #930 W2 — pre-declared bounded content-forward lexical rebalance.
  // Per-column weights are entry_id(0), name(4), description(3), tags(2),
  // hints(2), and content(3). This arm is measured independently before any
  // release pick; W1 remains the preferred qualifier if both pass.
  ...
  const sql = `
    ...
    WITH scored AS MATERIALIZED (
      ...
      SELECT e.id, bm25(entries_fts, 0, 4.0, 3.0, 2.0, 2.0, 3.0) AS bm25Score
    FROM entries_fts f
