...
import { getMarkdownFragmentContent, type IndexDocument } from "../../indexer/passes/metadata";
...
export function upsertEntry(
  ...
  const apply = (): number => {
    ...
    replaceFtsEntry(db, result.id, entry, getMarkdownFragmentContent(entry));
    return result.id;
  };
...
export function rekeyEntryInPlace(db: Database, opts: RekeyEntryOptions): number | null {
  ...
  db.transaction(() => {
    ...
    if (document) replaceFtsEntry(db, row.id, document, getMarkdownFragmentContent(document));
