...
export async function curateSearchResults(
  ...
  // intent nudges, score floor) and `preferBroadRootRepresentative` (family
  // collapse) entirely, taking the raw top-N of whatever order the hits
  // arrived in. Filtering the pool down to the requested type up front and
  ...
  const selectedStashHits = preferred.selected;
  const supportRefsByRef = preferred.supportRefsByRef;
