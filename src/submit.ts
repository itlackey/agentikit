...
import { type AgentikitAssetType, fetchWithRetry, isAssetType } from "./common";
...
export async function agentikitSubmit(options: AgentikitSubmitOptions = {}): Promise<SubmitResponse> {
  ...
  try {
    ...
    if (options.cleanupFork) {
      progress(
        `Fork cleanup deferred — the PR source branch lives on the fork. Run \`${cleanupCommand}\` after the PR is merged.`,
      );
    }
    ...
    if (forkCreated) {
      progress(
        `A fork was created at https://github.com/${username}/${REGISTRY_REPO} but the submit failed. You can delete it with: gh repo delete ${username}/${REGISTRY_REPO} --yes`,
      );
    }
    throw error;
  } finally {
...
export async function buildSubmitEntry(
  ...
  try {
    ...
    const promptedAssetTypes = await promptWithDefault(
      "Asset types (comma-separated)",
      defaultAssetTypes.join(", "),
      rl,
    );
...
async function resolveSubmitTarget(
  ...
  if (parsed.source === "git") {
    throw new Error(
      "`akm submit` does not support generic git URLs. Use a public npm package name or GitHub owner/repo ref instead.",
    );
  }
...
async function inferSubmitTargetFromDir(dir: string): Promise<{
  ...
  if (!packageJson) {
    throw new Error(
      "Unable to infer a public npm or GitHub ref from the current directory. Add a package.json or pass a ref explicitly.",
    );
  }
...
function readPackageJson(dir: string): PackageJsonLike | undefined {
  ...
  const assetTypes = Array.isArray(agentikit.assetTypes)
    ? agentikit.assetTypes.filter(
        (value): value is AgentikitAssetType => typeof value === "string" && isAssetType(value),
      )
    : undefined;
...
async function fetchRegistryManualEntries(): Promise<unknown[]> {
  ...
  if (!entriesResponse.ok) {
    throw new Error(
      `Failed to load ${MANUAL_ENTRIES_FILE} from ${REGISTRY_OWNER}/${REGISTRY_REPO} (${entriesResponse.status}).`,
    );
  }
...
function buildPlannedCommands(options: {
  ...
  if (options.cleanupFork) {
    commands.push(
      `# After PR is merged: ${formatCommand(["gh", "repo", "delete", `${options.username}/${REGISTRY_REPO}`, "--yes"])}`,
    );
  }
  return commands;
}

function buildPullRequestArgs(entry: RegistryKitEntry, username: string, branchName: string, body: string): string[] {
  return [
    "pr",
    "create",
    "--repo",
