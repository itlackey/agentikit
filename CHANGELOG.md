...
- **OpenViking registry provider**: `openviking` provider type for searching OpenViking servers via their REST API; add with `akm registry add <url> --provider openviking`
...
- **`--options` flag for `akm registry add`**: pass provider-specific JSON config (e.g., `--options '{"apiKey":"key"}'`)
