...
| **R-02** | **`directScript` fabricates synthetic task YAML and re-parses it.** A `uses: scripts/<ref>` workflow step is executed by writing a task document as a string — including a fake `@daily` schedule that exists only to satisfy R-06 — and running it back through `parseTaskV3Yaml`. | `src/workflows/ir/source-freeze-v4.ts:274-298` | The synthetic document is exactly:<br>`version: 3\nuses: <owned.ref>\nakm:\n  schedule: "@daily"\n`<br>with `filePath = \`${context.asset.path}#${source.id}\``, `workspaceRoot = context.asset.sourcePath`, `taskId = source.id`, `taskRef = \`${context.asset.ref}#${source.id}\``.<br>Pin it **observably**: a script step freezes to a script dispatch whose identity fields carry the `<workflow path>#<step id>` shape above. Also pin the hard invariant at `:296` — a non-script projection throws a bare `Error("direct script did not project as a script")` (exit 70, not a `UsageError`). | **P1b** (typed preparer) | A |
| **R-03** | **Nested-workflow targets are rejected at three separate sites, two with byte-identical messages.** | `src/workflows/source-ir/semantics.ts:141-146`; `src/workflows/ir/source-freeze-v4.ts:220-222`; `src/workflows/ir/source-freeze-v4.ts:237-239` | Pin **all three sites independently** — a later phase must not be able to remove one silently.<br>`semantics.ts`: `WorkflowSourceSemanticError`, code `nested-workflow-unsupported`, message `Nested workflow target "<value>" is unsupported in a workflow step.` (value JSON-stringified).<br>Both `source-freeze-v4.ts` sites: `UsageError`, code `INVALID_FLAG_VALUE`, message exactly `A workflow task step cannot compose a nested workflow target.` — `:220` fires on the parsed task document (`task.target.uses.kind === "workflow"`), `:237` on the prepared execution (`prepared.kind === "workflow"`). Each needs a fixture that reaches **that** site. | **P3** (child workflows) | A |
...
- [ ] **R-02** A `uses: scripts/<ref>` step freezes to a script dispatch; pin the `<workflow path>#<step id>` identity shape the synthetic document produces, and pin the bare-`Error` invariant at `source-freeze-v4.ts:296`.
- [ ] **R-03** Three separate tests, one per site: `semantics.ts:141-146` (code `nested-workflow-unsupported`), `source-freeze-v4.ts:220-222`, `source-freeze-v4.ts:237-239`. The two freeze-path messages are byte-identical — the tests must be distinguishable by **fixture**, not by message.
...
- [ ] R-03's three sites are covered by three distinct tests reaching three distinct code sites, distinguishable by fixture rather than by message text.
