...
| **B-51** | The D8 legacy read mapping in `run/task-history.ts` (`"prompt"` → `{kind:"command", engine}`, legacy `"command"` → `{kind:"shell"}`) | present | **SUPERSEDED 2026-09-01** — moved into state migration `025-task-history-vocabulary-backfill` (`src/core/state/migrations.ts`); deleted from this file and the two other read sites that had re-implemented it (`src/commands/health/improve-metrics.ts`, `src/commands/health/windows.ts`). See `docs/architecture/decisions/0005-task-result-vocabulary-and-legacy-read-mapping.md`'s 2026-09-01 entry. |
