...
- **#954** — decision 2's configurable `embedding.concurrency` was superseded
  by the dev-team field review before it shipped: the in-flight window
  ...
  `--parallel N`, vLLM) was left idle by the fixed concurrency. #9541 (before
  ...
  `embedding.concurrency` (bounded 1-16) reverses the "no knob" call above by
  owner decision, opt-in only for a server that genuinely serves parallel
  slots. #9541 also makes `embedding.timeoutMs` configurable (default 120s,
...
- **#955** — vector identity is keyed on the endpoint's reported response
  ...
  sample either way.
- **#9542** — pulled forward from a later milestone by owner decision on
  2026-09-09, alongside #9541 (both land via the same "embed" worktree/
  batch, #9541 first). Embedding salvage across full rebuilds and index-
