...
| D2 remove `show` view grammar | Not shipped |
| D3 remove `akm mv` | Not shipped |
| D4 conceptId-prefix browse | Not shipped |
| D5 remove `akm bundle` | **Shipped** |
| D6 free-form `type` | **Shipped** at runtime; CLI help text updated |
| D7 all six formats everywhere | Not shipped (`md`/`html` are health-only) |
| D8 gate improve autonomy | Not shipped (no gate exists; lanes are on by default) |
| D9 `--auto-accept` warn-and-ignore | Not shipped — the flag was removed outright instead |
...
**Status: NOT SHIPPED.** `normalizeShowArgv` still rewrites argv and `toc|section|lines|frontmatter|full` are still parsed and advertised. D1 shipped alongside it, so both spellings currently work.
...
**Status: NOT SHIPPED.** `src/cli.ts` still registers `mvCommand` and the full implementation still ships.
...
**Status: NOT SHIPPED.** `parseRefPrefixQuery` still implements only the `<type>:` grammar, validated against `placementTypes()`.
...
**Status: NOT SHIPPED.** `md` falls back to the JSON envelope and `html` throws, on every command except `akm health`.
...
**Status: NOT SHIPPED as described.** The flag was removed outright rather than warn-and-ignored; it is now silently absorbed, and the space-separated form is misread as the scope positional.
