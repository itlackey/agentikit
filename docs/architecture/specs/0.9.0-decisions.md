...
## D11 — OKF is the foundational format; adapters are extensions

**Decided.** Every item akm indexes is an OKF concept: identified by its path
within its bundle, typed by an open `type` read from its own frontmatter,
related by ordinary markdown links. Adapters project native formats *into* that
model rather than being alternatives to it. This amends normative spec §2,
which previously held that OKF "is not AKM's internal schema".

**Why.** akm already treats path-as-identity and open types as the core model
everywhere except one place — ref parsing — and that exception breaks OKF
support at the most basic level. A sweep with a conformant three-concept OKF
bundle found the primary documented flow non-functional: `akm search` prints
`akm show okfbundle//tables/customers` and that exact command fails, because
`parseRefInput` requires the first conceptId segment to be one of thirteen
`akm` placement subdirs. Five further misalignments follow from the same split
model (identity that changes when an optional `index.md` is deleted; a search
set and a `show` set that are disjoint; akm writes that state no `type`,
`title`, or `timestamp`; a default workspace that is never an OKF bundle;
resolved OKF links that are never persisted). Full evidence in
[okf-foundational.md](./okf-foundational.md).

Making OKF foundational is the smaller change: it deletes a special case rather
than adding one.

**Breaks.** Almost nothing, because the change is *widening*. Refs that resolve
today continue to resolve; refs that fail today (any conceptId outside the
thirteen-word vocabulary) start working. The one genuine change is that
index-less bundles stop getting a synthetic `knowledge/` segment injected into
their refs — those refs are already wrong, so it is a repair. `akm init` gains
a root `index.md`.

**Explicit non-goals.** Native files stay native — no adapter rewrites a Claude
command, a `SKILL.md`, or a `.env` into OKF shape. OKF does not become an asset
type. The `memories/` / `skills/` / `knowledge/` directory conventions stay;
they become a *default* for type rather than the *mechanism* for it.

**Normative text.** [`okf-foundational.md`](./okf-foundational.md);
[`ref.md`](./ref.md).
