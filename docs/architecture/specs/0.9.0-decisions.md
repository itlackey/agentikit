...
**Status: NOT SHIPPED** (one hazard fixed). `md` still falls back to the JSON envelope and `html` is still health-only. The write-then-throw ordering bug IS fixed: `--format html` is now rejected at startup, before a mutating command body runs.
