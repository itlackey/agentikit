...
**Status: NOT SHIPPED — and the gap is safety-relevant.** No `experimental.*` key exists; the mutating lanes (consolidate delete, memoryInference writes) are ON in the shipped `default` strategy.
