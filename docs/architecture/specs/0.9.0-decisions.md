...
## D11 — OKF is a first-class supported format, not AKM's content model

**Decided.** AKM supports OKF through a first-class built-in `okf` adapter.
OKF is not AKM's internal schema, default adapter, or universal type and
identity model. Native formats remain governed by their own adapters.
...
Fixing that boundary does not require redefining every indexed item as an OKF
concept. Doing so would conflict with native `.env`, YAML, script, Agent Skills,
Claude, OpenCode, LLM Wiki, and AKM stash semantics. The adapter and index
already provide the correct boundary: each adapter owns native semantics and
projects only the fields needed for cross-format search and reads.

**Required changes.** Ref resolution must accept the concept IDs emitted by the
selected adapter; OKF metadata and links must survive indexing; and writes to a
writable OKF source must use OKF-owned placement and serialization. These
requirements are scoped to OKF sources. The `akm` adapter keeps its existing
matcher-based classification, and `akm init` does not gain a root `index.md`.
