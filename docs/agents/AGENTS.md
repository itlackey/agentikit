...
| wiki | A page in a multi-wiki knowledge base. For any wiki task, run `akm wiki list`. `akm wiki ingest <name>` dispatches the configured agent engine (defaults.engine or `--engine`) to execute the wiki's ingest workflow. `akm wiki -h` for the full surface. |
| lesson | A distilled feedback lesson (`when_to_use` plus body). Read before applying related skills. Generated by the improve pipeline and promoted through the proposal queue. |
