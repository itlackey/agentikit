...
Ref-prefix queries (a conceptId prefix ending in `/`, optionally
bundle-qualified — `"memories/projectA/"`, `"team-catalog//skills/"`) return a
deterministic listing, not a relevance ranking. A complete ref without the
trailing slash (`memories/projectA/auth-tip`) stays an ordinary keyword search —
resolving a single ref is `akm show`'s job. Prefixes match conceptIds, so a ref
prefix copied from search output works as a query verbatim.
...
| wiki | `content` (an LLM Wiki page, shown like `knowledge`; the same `#fragment` selection works via its `bundle//conceptId` ref — see [LLM Wiki bundles](#llm-wiki-bundles)) |
