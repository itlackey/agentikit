...
being auto-promoted by threshold. The flag is no longer accepted. Remove it
from task definitions and scripts; see [proposal triage](#4-behavioral-notes)
for the explicit replacement.
...
Additive (non-breaking) singular aliases for `akm tasks`/`akm lessons` also
shipped in 0.8 as `akm task`/`akm lesson`. 0.9.0 retires the plural spellings
entirely (no alias): `akm task` is now the sole scheduling group, and
`akm lessons` was removed outright (see [§3](#3-removed-surfaces)).
...
| `akm tasks ...` | `akm task ...` | singular group; no alias |
| `akm tasks init` | (removed) | default task templates are now embedded and seeded by `akm setup` |
| `akm tasks enable` / `akm tasks disable` | (removed) | edit the task file's `enabled:` key, then `akm task sync` |
...
| `akm workflow validate` | `akm lint --type workflows` | |
...
| `akm improve canary` / `--refresh` | `bun scripts/refresh-canary-set.ts [--refresh]` | maintainer tooling, not a CLI verb |
...
| `akm setup --detect-only` / `--reset-recommended` | (removed) | `akm info` covers detection |
| `akm env set` / `akm env unset` | (removed) | edit the `.env` file directly, or ingest one with `env create --from-file` |
| `akm hints` (top-level) | `akm help agents` (`--full` for the long guide) | |
| `--source` on `search` / `curate` | `--from` | value rename too: `stash` → `local`, `both` → `all` |
| `--target` on `remember` / `clone` / `improve` | `--bundle` | `import`, `task add`/`run`/`sync`/`history`, and `proposal accept`/`diff`/`revert` **keep** `--target` |
