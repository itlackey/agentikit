...
6. After apply succeeds, run `akm tasks sync --rebind` with that same 0.9 binary
...
the disabled task; use `akm backup create --for 0.9.0` for an explicit migration
