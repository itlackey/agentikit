...
New task files use strict YAML v2. During `migrate apply`, valid 0.8 task files
with legacy workflow-target refs are surgically rewritten to canonical refs;
other fields and current refs remain byte-identical. The 0.9 reader continues to
normalize their missing/`1` version, prompt `profile:` field, permissive scalar
forms, and `akm improve --profile` commands in memory. Update user-authored files
...
list`, `sync`, and `doctor`. Reader normalization changes only the removed AKM
spellings; arbitrary shell commands are never rewritten.
