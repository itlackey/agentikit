...
An existing database with no `schema_migrations` table is never treated as a
fresh install. Ordinary commands reject it without writing. The explicit
`akm upgrade --force` path takes and verifies a descriptor-bound snapshot named
