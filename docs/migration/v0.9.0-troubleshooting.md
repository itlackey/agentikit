...
Both run on an already-current install, need no network for the state step,
and never install anything to reach it, so either is safe as a container
entrypoint: on a current database the state step is a no-op. (`akm upgrade
--state-only` is the same step with no release check at all, for installs
that cannot reach the network.) Immediately before
