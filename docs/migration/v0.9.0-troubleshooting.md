...
| `$STASH` | `~/akm` | working stash: assets, tasks, scripts, workflows, env files |

`akm config path --all` prints the resolved config, stash, cache, and index
...
`akm tasks doctor` reports the runtime `kind` and whether it is `eligible`
...
error on every scheduled run. Replace it with the stash path from
`akm config path --all` or `akm info`. Use `akm tasks history` to spot
...
3. Verify with `akm index`, `akm search`, and `akm show`, and lint the
   destination subtree rather than the whole stash.
...
`akm tasks history` can also show rows stuck in an active state from runs
...
On a Git-backed stash the usual cause is benign: a tracked file was deleted
...
the deletion in the stash repository and re-index:

```sh
git -C <stash-dir> status --short
akm index
```
...
1. Search the stash for the literal template phrases and remove only the
...
akm tasks run <quick-improve-task-id>
```
...
Keep scheduler environment files (`env/*.env` in the stash) at mode `600`
...
- `akm tasks history` shows no rows stuck in an active state.
