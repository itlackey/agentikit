...
- **Typed `inputs:` with defaults and `required:`.** v4 tasks can declare
  ...
  supply literal `inputs:` too. See
  [Tasks: Typed inputs and output](../reference/tasks.md#typed-inputs-and-output)
  for the full grammar, including the "no document-level enabled" trap for
  a `required: true` input with no default on a scheduled task.
...
```
# 0.9.2
Task v3 uses must be akm/command, a canonical commands/, workflows/, or
scripts/ asset ref. Agent/task/local/Docker/remote-action/ambiguous targets
are not executable.
```
...
**Resume caveat.** A reference binding's *resolved* value is not part of
the unit's durable input-identity hash — only the reference's own text
(`from: "steps.discover.output.files"`) is. In the ordinary case this is
harmless: a frozen plan never re-reads source, so the same reference
resolves the same way on every attempt. But journaled
`workflow_run_units` rows should be treated as read-only, the same as any
other durable run state — reaching in and altering an earlier,
already-completed step's journaled output before resuming a run could let
a later unit reuse a journaled row bound to a now-stale resolved value
without the durable hash catching the mismatch.
...
Both are read-only, zero-write, and secret-free by construction — neither
spawns anything, writes history, publishes a run, or prints a resolved
reference value, request content, script byte, or credential.
...
Its default text output and its `--format json` output are the same raw
JSON, byte for byte — there is no separate human-readable renderer. See
[Tasks: `akm task explain`](../reference/tasks.md#akm-task-explain).

## Diagnostics

`INVALID_FLAG_VALUE` no longer appears in task or workflow domain failures.
Every task-source, workflow-source, target-classification, and composition
failure now reports a phase-specific code:
...
The remaining `INVALID_FLAG_VALUE` sites in the task/workflow domains are
scalar CLI-argument parsing (a cron expression, a task id, a workflow
parameter flag) — genuine flag-value validation, not a task/workflow source
or composition failure. **Scripts branching on `code` for a task/workflow
domain error should switch on the specific code above** rather than
assuming `INVALID_FLAG_VALUE`. Exit codes are unchanged — every code above
is exit 2, same as before.
