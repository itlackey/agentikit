...
### `with:` on a task-composed step now rejects
...
error and no warning. 0.9.2 makes this fail closed instead: freezing that step
now throws
...
as a `UsageError` with code `COMPOSITION_INVALID` (exit 2). This fires for any
authored `with:` shape that survives decode — including an empty mapping
(`with: {}`) — not just a non-empty one. A step targeting a task with no
`with:` at all is unaffected and keeps freezing exactly as before.

Task-call inputs are not implemented in 0.9.2; that support arrives in a later
0.9.x release. Until then, fix a workflow that hits this by deleting the
step's `with:` block:
...
`with:` on `uses: akm/command` is a different, unaffected path — it is still
required to supply the builtin action's arguments and continues to work as
documented.
