...
flattens to top-level keys, and a `with:` authored on a non-`akm/command`
target converts to a declared `inputs:` entry where the value shape makes the
input type inferable. Task source v4 is purely additive: `version: 3`
sources keep parsing, running, and scheduling exactly as before, so this
migration is entirely optional.
...
or a `with:` shape too ambiguous to convert into a typed `inputs:`
declaration automatically. Apply validates the complete v4 replacement
through the real task source v4 parser before writing, and backs up each
original immediately before replacement — the same safety model the v2 → v3
migrator uses. `akm-migrate` is a separate executable from `akm` (installed
alongside it); this generation is not (yet) reachable through `akm migrate`.
