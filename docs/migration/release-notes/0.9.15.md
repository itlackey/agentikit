...
prior fixed 30s that cut off a slow local model server mid-response with no
retry). After 3 consecutive transport failures (never a batch rejected only
for exceeding the endpoint's context window), `akm index`'s embedding phase
now stops dispatching further requests and reports failure instead of
grinding through every remaining batch against a dead endpoint — batches
already committed are kept, and a rerun picks up where it left off.
