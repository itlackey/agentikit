...
`embedding.concurrency` is not, and has never been, a config field — it never
shipped despite appearing in some discussion of this release. Embedding
throughput is tuned by `embedding.batchSize` (documents per request) and
`embedding.maxTokens`/`contextLength` (token budget per request); the number of
requests in flight is fixed (not configurable).
