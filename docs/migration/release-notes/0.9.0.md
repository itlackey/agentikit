...
Live credential-backed agent/LLM calls, destructive migration crash injection,
and native macOS/Windows execution were not run from this Linux host. Their
deterministic paths remain covered by the full unit/integration suite; release
artifacts still require their normal platform CI/release checks.
