...
import { parseAssetRef, makeAssetRef } from "../src/stash-ref"
...
describe("makeAssetRef", () => {
  ...
  test("nested path — slashes stay literal", () => {
    expect(makeAssetRef("tool", "azure/container-apps/scale.sh"))
      .toBe("tool:azure/container-apps/scale.sh")
  })
  ...
  test("with local origin", () => {
    expect(makeAssetRef("tool", "deploy.sh", "local"))
      .toBe("local//tool:deploy.sh")
  })

  test("with npm origin — colons and @ stay literal", () => {
    expect(makeAssetRef("tool", "deploy.sh", "npm:@itlackey/openkit"))
      .toBe("npm:@itlackey/openkit//tool:deploy.sh")
  })

  test("with github shorthand origin", () => {
    expect(makeAssetRef("skill", "code-review", "itlackey/dimm-city-kit"))
      .toBe("itlackey/dimm-city-kit//skill:code-review")
  })

  test("with github prefixed origin", () => {
    expect(makeAssetRef("tool", "lint.sh", "github:owner/repo#v1.2"))
      .toBe("github:owner/repo#v1.2//tool:lint.sh")
  })

  test("nested name with origin", () => {
    expect(makeAssetRef("tool", "db/migrate/run.sh", "npm:@corp/db-tools"))
      .toBe("npm:@corp/db-tools//tool:db/migrate/run.sh")
  })
