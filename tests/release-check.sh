...
run_step \
	...
	run_step \
		...
		node -e 'const { spawnSync } = require("node:child_process"); const result = spawnSync(process.execPath, ["./dist/scripts/akm-migrate-node.js", "--help"], { encoding: "utf8" }); if (result.error || result.status !== 0) { console.error(result.error?.message ?? result.stderr); process.exit(1); } if (!result.stdout.includes("Inspect task-v2 files") || !result.stdout.includes("task v3")) { console.error("Packaged Node migrator does not expose the task-v2 to task-v3 boundary"); process.exit(1); }'
