...
describe("decodeTaskHistoryMetadata", () => {
  ...
  test("rejects v1 metadata, profile aliases, extras, and missing v2 fields", () => {
    expect(() => decodeTaskHistoryMetadata('{"durationMs":12,"profile":"old"}')).toThrow(/unsupported metadataVersion/);
    expect(() => decodeTaskHistoryMetadata({ metadataVersion: 1, durationMs: 12, detail: null })).toThrow(
      /unsupported metadataVersion: 1/,
    );
    ...
    expect(() => decodeTaskHistoryMetadata({ durationMs: 1, extra: true })).toThrow(/unsupported metadataVersion/);
  });
});
