 ...
 * (`akm upgrade/sync/clone/history`). Pins the JSON envelope (stdout payload
 * shape + the {ok:false,code} error envelope on stderr / exit code) for
 * representative subcommands, proving the extraction from cli.ts into
 * src/commands/sources-cli.ts is byte-identical.
 *
 * The `runSyncBody` git body and the `wasFormatValueConsumedAsName` workaround
 * moved with the cluster; the leaf handlers (all but `sync`) were migrated onto
 ...
 *
 * 0.9 CLI overhaul (S7): `list`'s `--kind` filter moved to `akm bundle list`
 * (src/commands/sources/bundle-cli.ts) — its envelope coverage moved with it;
 * see tests/commands/bundle-cli-envelope.test.ts.
 */
