...
function linearGolden(
  ...
  const unit = (id: string, instructions: string, source: (typeof sources)[number]) => ({
    ...
    invocation: { engine: "test-agent", model: "test-model", modelPresent: false, timeoutMs: 600_000 },
    ...
    steps: [
      {
        ...
        gate: {
          ...
          maxLoops: 1,
          judge: { engine: "test-llm", model: "test-model", modelPresent: false, timeoutMs: 600_000 },
        },
      },
      {
        stepId: "deploy",
