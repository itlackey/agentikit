...
describe("source commands and resolution", () => {
  // XDG_* / AKM_BUNDLE_DIR snapshot+restore is provided by tests/_preload.ts.
  ...
  beforeEach(() => {
    ...
    // Pair AKM_BUNDLE_DIR mutations below with XDG_DATA_HOME / XDG_STATE_HOME
  ...
  test("akmSearch resolves script run correctly for search path directories", async () => {
    ...
    process.env.AKM_BUNDLE_DIR = primaryStashDir;
...
`;
  ...
  test("akmInit returns created false when stash dir already exists", async () => {
    ...
    const origStashDir = process.env.AKM_BUNDLE_DIR;
    ...
    try {
      ...
      if (origStashDir === undefined) delete process.env.AKM_BUNDLE_DIR;
      else process.env.AKM_BUNDLE_DIR = origStashDir;
  ...
  test("akmSearch finds script assets with broad extensions", async () => {
    ...
    try {
      ...
      if (origStashDir === undefined) {
        ...
        process.env.AKM_BUNDLE_DIR = origStashDir;
      }
      fs.rmSync(stashDir, { recursive: true, force: true });
    }
  });
