...
// tests/asset-mutation-lease.test.ts), not the cross-process contention path.
// The spawned `sleep 5` exists purely to be that live foreign pid.
...
import { acquireAssetMutationLease } from "../../src/indexer/index-writer-lock";
...
describe("asset mutation lease (cross-process)", () => {
  test("try mode coalesces when another process holds the lease", async () => {
    ...
    try {
      ...
      const result = await acquireAssetMutationLease({ mode: "try", purpose: "background" });
