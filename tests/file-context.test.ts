...
describe("runMatchers", () => {
  ...
  test("smartMdMatcher detects 'agent' frontmatter as command signal at specificity 18", () => {
    ...
    writeFile(filePath, ["---", "agent: build", "description: Build the project", "---", "Build $ARGUMENTS."].join("\n"))
...
describe("Renderer", () => {
  ...
  test("getRenderer('knowledge-md') handles toc view", () => {
    ...
    writeFile(
      filePath,
      ["---", "title: Guide", "---", "# Introduction", "Welcome.", "", "## Setup", "Install.", "", "## Usage", "Use."].join("\n"),
    )
  ...
  test("getRenderer('knowledge-md') handles section view", () => {
    ...
    const renderer = getRenderer("knowledge-md")!
    const ctx = buildFileContext(root, filePath)
    const match = { type: "knowledge", specificity: 10, renderer: "knowledge-md", meta: { name: "guide.md", view: { mode: "section" as const, heading: "Setup" } } }
    const renderCtx = buildRenderContext(ctx, match, [root])
    const response = renderer.buildShowResponse(renderCtx)
  ...
  test("getRenderer('knowledge-md') handles lines view", () => {
    ...
    writeFile(
      filePath,
      ["# Intro", "Welcome.", "", "## Setup", "Install things."].join("\n"),
    )

    const renderer = getRenderer("knowledge-md")!
    const ctx = buildFileContext(root, filePath)
    const match = { type: "knowledge", specificity: 10, renderer: "knowledge-md", meta: { name: "guide.md", view: { mode: "lines" as const, start: 1, end: 2 } } }
    const renderCtx = buildRenderContext(ctx, match, [root])
    const response = renderer.buildShowResponse(renderCtx)
