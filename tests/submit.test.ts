...
import { agentikitSubmit, buildSubmitBranchName, buildSubmitEntry, slugifySubmitValue } from "../src/submit";
...
async function withMockedFetch<T>(handler: (url: string) => Response, run: () => Promise<T>): Promise<T> {
...
describe("agentikitSubmit", () => {
  test("dry run infers a public ref from package.json metadata", async () => {
    ...
    expect(
      result.commands?.some((command) => command.includes("gh repo fork itlackey/agentikit-registry --clone --remote")),
    ).toBe(true);
  });
  ...
  test("dry run fetches manual entries from raw.githubusercontent using slash-containing default branches", async () => {
      ...
      () =>
        withMockedFetch(
          (url) => {
            ...
            if (
              url === "https://raw.githubusercontent.com/itlackey/agentikit-registry/release/2026/manual-entries.json"
            ) {
              return new Response("[]", { status: 200 });
            }
            return new Response("not found", { status: 404 });
    ...
    expect(urls).toContain(
      "https://raw.githubusercontent.com/itlackey/agentikit-registry/release/2026/manual-entries.json",
    );
  ...
  test("full submit workflow uses gh and git commands in order", async () => {
    ...
    expect(
      result.commands?.some(
        (command) => command.includes("--body") && command.includes("## New registry entry: owner-kit"),
      ),
    ).toBe(true);
