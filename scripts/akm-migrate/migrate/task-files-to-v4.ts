...
/** Filesystem boundary for the explicit task-v3 to task-source-v4 converter. */
...
export interface ApplyTaskToV4Options {
  readonly backupRoot: string;
}

export interface AppliedTaskToV4Plan {
...
interface Snapshot {
  ...
  readonly identity: TaskToV4FilesystemIdentity;
}
...
function identity(filePath: string, kind: "file" | "directory"): TaskToV4FilesystemIdentity {
...
function sameIdentity(left: TaskToV4FilesystemIdentity, right: TaskToV4FilesystemIdentity): boolean {
  return (
...
function walkTasks(root: TaskToV4Root, tasksDir: string, out: TaskToV4FileInput[]): void {
...
export function inspectTaskToV4Files(roots: readonly TaskToV4Root[]): TaskToV4FileInput[] {
...
export function taskMigrationBackupPathV4(backupRoot: string, filePath: string): string {
...
function assertUnchanged(change: TaskToV4Changed): Snapshot {
...
export function applyTaskToV4MigrationPlan(
  ...
  for (const change of changes) {
    writeDurable(taskMigrationBackupPathV4(options.backupRoot, change.filePath), change.before, change.mode, true);
  }
  ...
  try {
    ...
    for (const change of [...replaced].reverse()) {
      ...
      replaceAtomically(
        change.filePath,
        fs.readFileSync(taskMigrationBackupPathV4(options.backupRoot, change.filePath)),
        change.mode,
      );
    }
    throw cause;
